...
//Loop over the 'in' gray scale array and create a binary (0,1) valued image 
//'out'
...
int component_labeling(unsigned char **binary_image, int **label, int height, int width) 
{
    ...
    for(int i = 0; i < height; i++)
    {
        for(int j = 0; j < width; j++)
        {
            ...
            // if the binary image is white and it has not been discovered 
            //before
            if(binary_image[i][j] == 1 && label[i][j] == 0)
            {
                ...
                while(!q.is_empty())
                {
                    ...
                    //the if statement is so that it is within bounds, that it 
                    //is white, and that it is unlabeled
                    //for top pixel
                    if((removed.row-1 >= 0) && binary_image[removed.row-1][removed.col] == 1 && 
                    ...
                    //for the pixel to the bottom of the removed pixel
                    if((removed.row+1 < height) && binary_image[removed.row+1][removed.col] == 1 && 
                        label[removed.row+1][removed.col] != current_label)
                    {
                        label[removed.row+1][removed.col] = current_label;
                    ...
                    //for the pixel to the left of the removed pixel
                    if((removed.col-1 >= 0) && binary_image[removed.row][removed.col-1] == 1 && 
                    ...
                    //for the pixel to the right of the removed pixel
                    if((removed.col+1 < width) && binary_image[removed.row][removed.col+1] == 1 && 
                        label[removed.row][removed.col+1] != current_label)
                    {
                        label[removed.row][removed.col+1] = current_label;
    ...
    //as current_label will be incremented one extra time at the end
    return current_label-1;
}

//First make num_segments random colors to use for coloring the labeled parts 
//of the image. Then loop over the labeled image using the label to index into 
//your random colors array. Set the rgb_pixel to the corresponding color, or 
//set to black if the pixel was unlabeled.
#ifndef AUTOTEST
void label2RGB(int **labeled_image, unsigned char ***rgb_image,
    int num_segments,int height,int width)
{
    //from 1 to num_segments inclusive because current_label starts at 1
