...
void rgb2gray(unsigned char ***in,unsigned char **out,int height,int width) 
{
    for(int i = 0; i<height; i++)
    {
        for(int j = 0; j<width; j++)
        {
            out[i][j] = (unsigned char)(in[i][j][0]*0.2989 + in[i][j][1]*0.5870 
                + in[i][j][2]*0.1140);   
        }
    }
}
        ...
        for(int j = 0; j < width; j++)
        {
            ...
            if(binary_image[i][j] == 1 && label[i][j] == 0)
            {
                ...
                while(!q.is_empty())
                {
                    ...
                    //for top pixel
                    if((removed.row-1 >= 0) && 
                        binary_image[removed.row-1][removed.col] == 1 && 
                    ...
                    if((removed.col-1 >= 0) && 
                        binary_image[removed.row][removed.col-1] == 1 && 
